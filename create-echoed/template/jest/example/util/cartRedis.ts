...
const getRedisPortInDocker = async (): Promise<string> => {
  ...
  const containerId = containers.find((container) =>
    container.Names.includes(`/${REDIS_CONTAINER_NAME}`),
  )?.Id;
  if (!containerId) {
  ...
  if (!portSetting) {
    throw new Error(
      `Port not found for container. container: ${REDIS_CONTAINER_NAME}, containerId: ${containerId}, network: ${REDIS_NETWORK_KEY}`,
    );
  }
